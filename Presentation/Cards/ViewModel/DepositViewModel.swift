...
final class DepositViewModel {
    ...
    init(
        ...
        self.depositUseCase = DepositUseCase(cardRepository: cardRepository)
    ...
    func deposit(amount: Double) {
        ...
        Task {
            ...
            do {
                let response = try await depositUseCase.execute(userId: userId, amount: amount, currency: currency)
                
                if response.success {
                    let newBalance = response.newBalance ?? currentBalance + amount
                    await MainActor.run {
                        self.currentBalance = newBalance
                        self.onSuccess?("Depósito realizado correctamente", newBalance)
                        self.onLoadingChange?(false)
                    }
                } else {
                    await MainActor.run {
                        self.onError?(response.error ?? "Error al depositar")
                        self.onLoadingChange?(false)
                    }
                }
            } catch {
